...
vi.mock('ethers', () => {
  ...
  class Contract {
    ...
    constructor(addr: string, abi: any, provider: any) { this.addr = addr; this.abi = abi; this.provider = provider; }
    connect(signer: any) { this.signer = signer; return this; }
    async asset() { return '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913'; }
    ...
    async allowance(_owner: string, _spender: string) { return 2n ** 255n; }
  ...
  function parseUnits(value: string, decimals: number) {
    ...
    const factor = decimals === 6 ? 1e6 : decimals === 18 ? 1e18 : Math.pow(10, decimals);
...
describe('baseusd:add command', () => {
  ...
  it('performs dry-run and prints estimates without sending tx', async () => {
    ...
    expect(ConsoleMock.log).toHaveBeenCalledWith(expect.stringContaining('--- baseUSD Add (dry-run) ---'));
    expect(ConsoleMock.log).toHaveBeenCalledWith(expect.stringContaining('Expected shares (baseUSD):'));
  ...
  it('submits live tx when not dry-run', async () => {
    ...
    expect(ConsoleMock.log).toHaveBeenCalledWith(expect.stringContaining('baseUSD Add (result)'));
  });
});