...
const { mockSendMessage, mockShutdown, mockCreateEmbedMessage } = (await import(
  "../../src/api/discord/discordService.js"
)) as any;
...
describe("confidenceCalibrate command", () => {
  ...
  beforeEach(() => {
    ...
    mockComputeCalibration.mockImplementation(
      async (market: string, days: number) => {
        return {
          ...mockCalibrationData,
          market,
          windowDays: days,
  ...
  describe("successful calibration", () => {
    it("should compute and save calibration with valid inputs", async () => {
      ...
      expect(output).toContain(
        "Saved to database: confidence_calibrations table",
      );
    });
  ...
  describe("dry-run mode", () => {
    it("should compute but not save calibration in dry-run mode", async () => {
      ...
      expect(output).toContain(
        "Calibration computed but NOT saved to database",
      );
  ...
  describe("error handling", () => {
    ...
    it("should handle no recommendations error", async () => {
      ...
      expect(errorOutput).toContain("💡 Hints:");
      expect(errorOutput).toContain(
        "No trade recommendations found in database",
      );
      expect(errorOutput).toContain(
        "npm run dev -- trade:recommend -m ETH --db",
      );
  ...
  describe("output formatting", () => {
    it("should format dates correctly", async () => {
      ...
      expect(output).toMatch(
        /Date Range: \d{2}\/\d{2}\/\d{4} - \d{2}\/\d{2}\/\d{4}/,
      );
    });
    ...
    it("should display market symbol in save confirmation", async () => {
      ...
      expect(output).toContain(
        "npm run dev -- trade:recommend -m ETH --calibrate",
      );
    });
  });
  ...
  describe("Discord notifications", () => {
    ...
    it("should send Discord notification on significant improvement", async () => {
      ...
      expect(embedBuilder.addTitle).toHaveBeenCalledWith(
    ...
    it("should handle Discord failure gracefully", async () => {
      ...
      const warnOutput = mockConsoleWarn.mock.calls
        .map((call) => call[0])
        .join("\n");
    ...
    it("should include all metrics in Discord embed", async () => {
      ...
      const embedBuilder = mockCreateEmbedMessage.mock.results[0].value;
      expect(embedBuilder.addDescription).toHaveBeenCalledWith(
        "Market: **BTC**",
      );
