...
describe("userProtocol command", () => {
  ...
  beforeEach(() => {
    ...
    UserProtocolService.getPoolFriendlyName = vi
      .fn()
      .mockImplementation((protocolId, poolId) => {
        if (
          protocolId === "base_flex" &&
          poolId === "0x053fa05d34c51afc5cb9f162fab3fd675ac06119"
        ) {
          return "FLP";
        }
        return undefined;
  ...
  it("should display an error if protocol ID is missing", async () => {
    ...
    expect(ConsoleMock.error).toHaveBeenCalledWith(
      "Protocol ID parameter is required",
    );
  ...
  it("should output JSON when json option is true", async () => {
    ...
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      JSON.stringify(mockUserProtocolResponse, null, 2),
    );
