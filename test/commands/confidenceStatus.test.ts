...
describe('confidenceStatus', () => {
  ...
  const setupMocks = (data: CalibrationData | null, timestamp: number | null) => {
    vi.mocked(ConfidenceCalibrationService.prototype.getLatestCalibration).mockResolvedValue(data);
    vi.mocked(ConfidenceCalibrationService.prototype.getLatestCalibrationTimestamp).mockResolvedValue(timestamp);
  };
  ...
  describe('single market status', () => {
    ...
    it('should display missing calibration status', async () => {
      ...
      expect(output).toContain('Run: npm run dev -- confidence:calibrate -m BTC');
    });
  ...
  describe('all markets status', () => {
    it('should display status for multiple markets', async () => {
      const btcMock = createMockCalibration(Date.now(), { market: 'BTC', correlation: 0.35 });
      const ethMock = createMockCalibration(Date.now() - 10 * 24 * 60 * 60 * 1000, { market: 'ETH', correlation: 0.15 });
      ...
      vi.mocked(ConfidenceCalibrationService.prototype.getLatestCalibrationTimestamp)
    ...
    it('should display summary with all healthy', async () => {
      const btcMock = createMockCalibration(Date.now(), { market: 'BTC', correlation: 0.35 });
      const ethMock = createMockCalibration(Date.now(), { market: 'ETH', correlation: 0.30 });
    ...
    it('should display warning recommendation for single market', async () => {
      ...
      const mock = createMockCalibration(Date.now(), { correlation: 0.15 });
      vi.mocked(ConfidenceCalibrationService.prototype.getLatestCalibration).mockResolvedValue(mock.data);
      vi.mocked(ConfidenceCalibrationService.prototype.getLatestCalibrationTimestamp).mockResolvedValue(mock.timestamp);
  ...
  describe('formatting', () => {
    ...
    it('should format win rate gap with sign and pp', async () => {
      const mock = createMockCalibration(Date.now(), { highConfWinRate: 0.65, lowConfWinRate: 0.45 });
      setupMocks(mock.data, mock.timestamp);
    ...
    it('should format negative gap correctly', async () => {
      const mock = createMockCalibration(Date.now(), { highConfWinRate: 0.45, lowConfWinRate: 0.65 });
      setupMocks(mock.data, mock.timestamp);
  ...
  describe('error handling', () => {
    it('should handle service errors gracefully', async () => {
      vi.mocked(ConfidenceCalibrationService.prototype.getLatestCalibration).mockRejectedValue(
  ...
  describe('health thresholds', () => {
    ...
    it('should categorize r=0.20 as warning (boundary)', async () => {
      const mock = createMockCalibration(Date.now(), { correlation: 0.20 });
      setupMocks(mock.data, mock.timestamp);
