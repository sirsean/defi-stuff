...
describe('history command', () => {
  ...
  it('should get all records when no options are specified', async () => {
    ...
    expect(mockGetByType).toHaveBeenCalledWith('0x1234567890abcdef', BalanceType.TOTAL);
    expect(mockGetByDate).toHaveBeenCalledWith('0x1234567890abcdef', '2023-05-15');
    expect(consoleLog).toHaveBeenCalledWith('Found 1 historical daily records:');
  });
  ...
  it('should display a message when no records are found', async () => {
    ...
    expect(mockGetByType).toHaveBeenCalledWith('0x1234567890abcdef', BalanceType.TOTAL);
    expect(consoleLog).toHaveBeenCalledWith('No historical data found for the specified criteria.');
  });
