...
vi.mock("ethers", () => {
      ...
      const fn: any = (..._args: any[]) => ({
        ...
        wait: async () => ({
          ...
          logs: mockState.makeLogsToMe
            ? [
                {
                  address: mockState.usdcAddress,
                  topics: [
                    mockState.transferTopic,
                    "0x",
                    "0x" + "0".repeat(24) + mockState.signerAddress.slice(2),
                  ],
                  data: "0x" + mockState.logsToMe.toString(16),
                },
              ]
            : [],
        }),
      });
      fn.populateTransaction = (..._args: any[]) => ({
        data: mockState.populatedData,
      });
      return fn;
    }
    async decimals() {
      return mockState.usdcDecimals;
    }
    async balanceOf(_addr: string) {
      return mockState.preBalance;
    }
    connect(_signer: any) {
      return this;
    }
  ...
  function keccak256(_bytes: any) {
    return mockState.transferTopic;
  }
  function toUtf8Bytes(_s: string) {
    return _s;
  }
  return {
    JsonRpcProvider,
...
describe("flpCompound command", () => {
  ...
  it("performs dry-run and prints estimates without sending tx", async () => {
    ...
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      expect.stringContaining("--- Flex FLP Compound (dry-run) ---"),
    );
  ...
  it("submits live tx and reports gas and USDC received", async () => {
    ...
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      expect.stringContaining("USDC received:"),
    );
  });
});
