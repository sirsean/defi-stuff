...
describe("chart command", () => {
  ...
  it("should generate both chart types by default", async () => {
    ...
    mockChartGenerator.generatePortfolioChart.mockResolvedValue(
      "charts/portfolio-full-7d.png",
    );
    mockChartGenerator.generateSimplifiedChart.mockResolvedValue(
      "charts/portfolio-simple-7d.png",
    );
    ...
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      "✅ Found data for 1 metrics over 2 days",
    );
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      "📈 Generating full portfolio chart...",
    );
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      "📊 Generating simplified chart...",
    );
    expect(ConsoleMock.log).toHaveBeenCalledWith(
  ...
  it("should use custom days parameter", async () => {
    ...
    expect(mockChartDataService.getChartData).toHaveBeenCalledWith(
      "0xtest",
      14,
    );
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      "📊 Fetching 14 days of data for 0xtest...",
    );
  });
  ...
  it("should handle no data gracefully", async () => {
    ...
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      "💡 Make sure you have run the daily command with --db flag to save historical data.",
    );
    expect(mockChartGenerator.generatePortfolioChart).not.toHaveBeenCalled();
    expect(mockChartGenerator.generateSimplifiedChart).not.toHaveBeenCalled();
  });
  ...
  it("should handle errors and provide helpful messages", async () => {
    ...
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      "💡 Tip: Set WALLET_ADDRESS in your .env file or use the -a option to specify an address",
    );
  ...
  it("should provide helpful error message for connection errors", async () => {
    ...
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      "💡 Tip: Make sure your database is set up and migrations have been run",
    );
  ...
  it("should provide helpful error message for canvas errors", async () => {
    ...
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      "💡 Tip: Make sure chartjs-node-canvas is properly installed",
    );
    expect(ConsoleMock.log).toHaveBeenCalledWith(
  ...
  it("should display data summary when charts are generated", async () => {
    ...
    expect(ConsoleMock.log).toHaveBeenCalledWith("\n📊 Data Summary:");
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      "   • Total USD: 100000.00 - 105000.00",
    );
    expect(ConsoleMock.log).toHaveBeenCalledWith(
      "   • Auto USD: 25000.00 - 30000.00",
    );
  });
});
