...
vi.mock(
  "../../src/api/debank/userProtocolService.js",
      ...
      UserProtocolService: vi.fn().mockImplementation(() => ({
        getUserProtocolData: vi.fn().mockImplementation((protocolId) => {
          ...
          return Promise.reject(
            new Error(`No data for protocol ${protocolId}`),
          );
        }),
        setWalletAddress: vi.fn(),
      })),
...
describe("daily command", () => {
  ...
  it("should use address from parameter when provided", async () => {
    ...
    const balanceServiceModule = await import(
      "../../src/api/debank/balanceService.js"
    );
    const userProtocolServiceModule = await import(
      "../../src/api/debank/userProtocolService.js"
    );
  ...
  it("should not save to database when --db flag is not set", async () => {
    ...
    const dbServiceModule = await import(
      "../../src/db/balanceRecordService.js"
    );
  ...
  it("should save to database when --db flag is set", async () => {
    ...
    const hasCorrectWalletAddress = records.every(
      (record) => record.wallet_address === walletAddress,
    );
  ...
  it("should send to Discord when --discord flag is set and properly shutdown connection", async () => {
    ...
    const refreshedModule = await import(
      "../../src/api/discord/discordService.js"
    );
  ...
  it("should handle errors", async () => {
    ...
    balanceServiceModule.BalanceService.mockImplementation(() => ({
      getUserBalanceWithThreshold: vi
        .fn()
  ...
  it("should handle database errors without crashing the application", async () => {
    ...
    mockSaveBalanceRecords.mockRejectedValueOnce(
    ...
    try {
      ...
      expect("Application crashed").toBe(
        "Application should handle DB errors gracefully",
      );
    }
