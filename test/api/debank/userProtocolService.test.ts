...
import {
  UserProtocolService,
  PROTOCOL_POOL_NAMES,
} from "../../../src/api/debank/userProtocolService.js";
...
describe("UserProtocolService", () => {
  ...
  describe("setWalletAddress", () => {
    it("should update the wallet address", () => {
      ...
      const getUserProtocolSpy = vi.spyOn(
        userProtocolService["debankClient"],
        "getUserProtocol",
      );
  ...
  describe("getPoolFriendlyName", () => {
    it("should return a protocol-specific pool name when available", () => {
      const name = UserProtocolService.getPoolFriendlyName(
        "tokemak",
        "0x726104cfbd7ece2d1f5b3654a19109a9e2b6c27b",
      );
