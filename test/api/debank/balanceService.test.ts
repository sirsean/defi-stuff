...
describe('BalanceService', () => {
  ...
  describe('getUserTotalBalance', () => {
    it('should return user balance data from the client', async () => {
      ...
      (mockClient.getUserTotalBalance as any).mockResolvedValue(mockUserTotalBalanceResponse);
  ...
  describe('getUserBalanceWithThreshold', () => {
    it('should filter chains by the default threshold (1000)', async () => {
      ...
      const result = await balanceService.getUserBalanceWithThreshold(userAddress);

      // Assert: Verify only chains with at least 1000 USD are included
      expect(result.total_usd_value).toEqual(mockUserTotalBalanceResponse.total_usd_value);
    ...
    it('should filter chains by a custom threshold', async () => {
      ...
      const result = await balanceService.getUserBalanceWithThreshold(userAddress, 40000);
      ...
      expect(result.chain_list.map(chain => chain.id)).toEqual(['eth', 'base', 'arbitrum']);
    });
    ...
    it('should sort chains by USD value in descending order', async () => {
      ...
      expect(result.chain_list.map(chain => chain.id)).toEqual(['eth', 'base', 'arbitrum', 'polygon']);
      expect(result.chain_list[0].usd_value).toBeGreaterThan(result.chain_list[1].usd_value);
      expect(result.chain_list[1].usd_value).toBeGreaterThan(result.chain_list[2].usd_value);
      expect(result.chain_list[2].usd_value).toBeGreaterThan(result.chain_list[3].usd_value);
    });
  });
});