...
import { TextMessageBuilder, EmbedMessageBuilder } from '../../../src/api/discord/messageFormatters';
...
describe('DiscordService', () => {
  ...
  describe('initialize', () => {
    ...
    it('should throw an error if channel retrieval fails', async () => {
      // Arrange
      (discordClient.getChannel as jest.Mock).mockRejectedValueOnce(new Error('Failed to get channel'));
      
      // Act & Assert
      await expect(discordService.initialize()).rejects.toThrow('Failed to get channel');
  ...
  describe('sendMessage', () => {
    ...
    it('should send a message from EmbedMessageBuilder', async () => {
      ...
      expect(mockChannel.send).toHaveBeenCalledWith({ embeds: [mockEmbedInstance] });
    });
    ...
    it('should throw an error if send fails', async () => {
      ...
      // Act & Assert
      await expect(discordService.sendMessage('Test message')).rejects.toThrow('Failed to send');
      expect(console.error).toHaveBeenCalled();
    });
    ...
    it('should throw an error if channel is not available', async () => {
      ...
      // Act & Assert
      await expect(newService.sendMessage('Test message')).rejects.toThrow('Discord channel not available');
    });
  });
