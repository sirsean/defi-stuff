...
describe("AbiService", () => {
  ...
  beforeEach(() => {
    ...
    mockExplorerClient = (BlockchainExplorerClient as any).mock.results[0]
      .value;
  });
  ...
  describe("getContractAbi", () => {
    it("should fetch and parse contract ABI for a regular contract", async () => {
      ...
      mockExplorerClient.getContractABI.mockResolvedValue(
        mockContractAbiResponse.result,
      );
      mockExplorerClient.getContractSourceCode.mockResolvedValue(
        mockContractSourceCodeResponse.result[0],
      );
      ...
      expect(mockExplorerClient.getContractABI).toHaveBeenCalledWith(
        testAddress,
      );
      expect(mockExplorerClient.getContractSourceCode).toHaveBeenCalledWith(
        testAddress,
      );
    });
    ...
    it("should fetch implementation ABI for a proxy contract", async () => {
      ...
      mockExplorerClient.getContractSourceCode.mockResolvedValue(
        mockProxySourceCodeResponse.result[0],
      );
    ...
    it("should use a different chain when specified", async () => {
      ...
      vi.mocked(BlockchainExplorerClient).mockImplementationOnce(() => ({
        getContractABI: vi
          .fn()
          .mockResolvedValue(mockContractAbiResponse.result),
        getContractSourceCode: vi
          .fn()
          .mockResolvedValue(mockContractSourceCodeResponse.result[0]),
  ...
  describe("getContractAbiJson", () => {
    it("should return ABI as formatted JSON string", async () => {
      ...
      // Act: Call the method being tested
      const result = await abiService.getContractAbiJson({
        address: testAddress,
      });

      // Assert: Verify we get properly formatted JSON
      const expected = JSON.stringify(
        JSON.parse(mockContractAbiResponse.result),
        null,
        2,
      );
  ...
  describe("checkForProxyContract", () => {
    it("should identify a proxy contract from explorer metadata", async () => {
      ...
      mockExplorerClient.getContractABI.mockResolvedValue(
        mockImplementationAbiResponse.result,
      );
      ...
      expect(result.implementationAbi).toEqual(
        JSON.parse(mockImplementationAbiResponse.result),
      );
    });
    ...
    it("should handle errors gracefully", async () => {
      ...
      mockExplorerClient.getContractSourceCode.mockRejectedValue(
        new Error("API error"),
      );
