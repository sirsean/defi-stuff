...
import { BalanceRecordService, BalanceType } from '../../src/db/balanceRecordService.js';
...
describe('ChartDataService', () => {
  ...
  beforeEach(() => {
    ...
    (BalanceRecordService as any).mockImplementation(() => mockBalanceRecordService);
  ...
  describe('getChartData', () => {
    ...
    it('should format chart data correctly with valid records', async () => {
      ...
      mockBalanceRecordService.getBalanceRecordsByDateRange.mockResolvedValue(mockRecords);
      ...
      const totalUsdDataset = result.datasets.find(d => d.label === 'Total USD');
      expect(totalUsdDataset).toBeDefined();
      expect(totalUsdDataset?.data).toEqual([100000, 105000]);

      // Should have Auto USD dataset
      const autoUsdDataset = result.datasets.find(d => d.label === 'Auto USD');
    ...
    it('should use environment variable when no address provided', async () => {
      ...
      expect(mockBalanceRecordService.getBalanceRecordsByDateRange).toHaveBeenCalledWith(
    ...
    it('should throw error when no address provided and no environment variable', async () => {
      ...
      await expect(chartDataService.getChartData(undefined, 7))
        .rejects.toThrow('No wallet address provided and WALLET_ADDRESS environment variable is not set');
    });
    ...
    it('should aggregate ETH values correctly', async () => {
      ...
      const autoEthDataset = result.datasets.find(d => d.label === 'Auto ETH');
      const dineroEthDataset = result.datasets.find(d => d.label === 'Dinero ETH');
    ...
    it('should aggregate rewards correctly', async () => {
      ...
      const rewardsDataset = result.datasets.find(d => d.label === 'Total Rewards');
