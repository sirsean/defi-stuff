...
import {
  validateCalibration,
  ValidationResult,
} from "../../scripts/validate-calibration.js";
  ...
  describe("validateCalibration", () => {
    ...
    describe("error handling", () => {
      it("should handle errors gracefully when computeCalibration reports insufficient data", async () => {
        ...
        // Act & Assert
        await expect(
          validateCalibration({ market: "BTC", days: 60 }),
        ).rejects.toThrow(
          "Insufficient data: need at least 10 trades, found 3",
        );
      ...
      it("should propagate other errors during validation", async () => {
        ...
        // Act & Assert
        await expect(
          validateCalibration({ market: "ETH", days: 90 }),
    ...
    describe("validation result structure", () => {
      ...
      it("should correctly compute improvement metrics", async () => {
        ...
        const expectedGapImprovement = result.calibratedGap - result.rawGap;

        expect(result.correlationImprovement).toBeCloseTo(
          expectedCorrelationImprovement,
          5,
        );
