...
try {
  const result = execSync(
    `launchctl list | grep com.defi-stuff.confidence-calibration`,
  ).toString();
  console.log(
    "Job is loaded and scheduled to run every Sunday at 6:00 AM CT (11:00 AM UTC)",
  );
  console.log(result);
} catch (error) {
  console.error(
...
console.log(
  "Setup complete! The confidence calibration command will run automatically every Sunday at 6:00 AM CT.",
);
console.log(
  `Logs will be written to: ${LOGS_DIR}/confidence-calibration-output.log and ${LOGS_DIR}/confidence-calibration-error.log`,
);
console.log(
  "\nIMPORTANT: Run 'npm run build' to compile TypeScript before the first scheduled run.",
);
