...
async function validateCalibration(
  ...
  try {
    ...
    const calibration = await calibrationService.computeCalibration(
      market,
      days,
    );
    console.log(
      `  ✓ Computed calibration from ${calibration.sampleSize} recommendations`,
    );
    console.log("");

    // Step 2: Apply calibration retroactively
    console.log(
      "Step 2: Applying calibration to historical recommendations...",
    );
    const { rawMetrics, calibratedMetrics } =
      await applyCalibrationRetroactively(calibration, calibrationService);
    ...
    const correlationImprovement =
      calibratedMetrics.correlation - rawMetrics.correlation;
    ...
    console.log("");
    console.log(
      `  Correlation:           ${formatCorrelation(rawMetrics.correlation)}`,
    );
    console.log(
      `  High Win Rate (≥0.7):  ${formatPercent(rawMetrics.highWinRate)}`,
    );
    console.log(
      `  Low Win Rate (<0.7):   ${formatPercent(rawMetrics.lowWinRate)}`,
    );
    console.log(
      `  Gap:                   ${formatPercent(rawMetrics.gap)} percentage points`,
    );
    console.log("");
    ...
    console.log("");
    console.log(
      `  Correlation:           ${formatCorrelation(calibratedMetrics.correlation)}`,
    );
    console.log(
      `  High Win Rate (≥median): ${formatPercent(calibratedMetrics.highWinRate)}`,
    );
    console.log(
      `  Low Win Rate (<median):  ${formatPercent(calibratedMetrics.lowWinRate)}`,
    );
    console.log(
      `  Gap:                   ${formatPercent(calibratedMetrics.gap)} percentage points`,
    );
    console.log("");
    console.log(
      `  Note: Using median calibrated confidence as threshold instead of 0.7`,
    );
    console.log("");
    ...
    console.log("");
    console.log(
      `  Correlation Change:    ${correlationImprovement >= 0 ? "+" : ""}${correlationImprovement.toFixed(3)}`,
    );
    console.log(
      `  Gap Change:            ${gapImprovement >= 0 ? "+" : ""}${formatPercent(gapImprovement)} percentage points`,
    );
    console.log("");
    ...
    if (correlationImprovement < 0.05) {
      issues.push(
        `Correlation improvement (${correlationImprovement.toFixed(3)}) is below target (≥0.05)`,
      );
    }

    if (calibratedMetrics.highWinRate <= calibratedMetrics.lowWinRate) {
      issues.push(
        "Calibrated high confidence win rate not exceeding low confidence",
      );
    }
    ...
    if (passes) {
      ...
      console.log("");
      console.log(
        "  Calibration successfully improves confidence score accuracy.",
      );
...
async function applyCalibrationRetroactively(
  ...
  const cutoffTimestamp =
    Date.now() - calibration.windowDays * 24 * 60 * 60 * 1000;
  ...
  for (let i = 0; i < recommendations.length - 1; i++) {
    ...
    const calibratedConfidence = service.applyCalibration(
      rawConfidence,
      calibration,
    );
  ...
  const sortedCalibrated = [
    ...outcomes.map((o) => o.calibratedConfidence),
  ].sort((a, b) => a - b);
  const medianCalibrated =
    sortedCalibrated[Math.floor(sortedCalibrated.length / 2)];
...
async function main() {
  ...
  if (marketIndex === -1 || marketIndex === args.length - 1) {
    ...
    console.error("");
    console.error(
      "Usage: npm run validate:calibration -- --market <MARKET> [--days <DAYS>]",
    );
    console.error("");
  ...
  const days =
    daysIndex !== -1 && daysIndex < args.length - 1
