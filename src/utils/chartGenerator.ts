...
export class ChartGenerator {
  ...
  async generatePortfolioChart(
    ...
    const imageBuffer =
      await this.chartJSNodeCanvas.renderToBuffer(configuration);
  ...
  async generateMultiPanelChart(
    ...
    const panels = await this.createChartPanels(
      chartData,
      panelWidth,
      panelHeight,
    );
  ...
  private async createChartPanels(
    ...
    const totalUsdDataset = chartData.datasets.find(
      (d) => d.label === "Total USD",
    );
    if (totalUsdDataset) {
    ...
    const autoEthDataset = chartData.datasets.find(
      (d) => d.label === "Auto ETH",
    );
    ...
    const autoUsdDataset = chartData.datasets.find(
      (d) => d.label === "Auto USD",
    );
    const baseUsdDataset = chartData.datasets.find(
      (d) => d.label === "Base USD",
    );
    if (autoUsdDataset && baseUsdDataset) {
      // Create combined Auto + Base USD dataset
      const combinedUsdData = autoUsdDataset.data.map(
        (autoValue, index) => autoValue + baseUsdDataset.data[index],
      );
