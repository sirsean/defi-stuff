...
export async function balance(
  ...
  try {
    ...
    balanceData.chain_list.forEach((chain) => {
      console.log(
        `${chain.name} (${chain.id}): $${chain.usd_value.toLocaleString()}`,
      );
    });
  } catch (error) {
    if (
      error instanceof Error &&
      error.message.includes("No balance data found")
    ) {
      console.error(
        "No balance data found for this wallet address. This could be because:",
      );
      console.error(
        "- The wallet has no balances on chains that Debank supports",
      );
      console.error(
        "- The wallet address is not valid or has not been indexed by Debank yet",
      );
