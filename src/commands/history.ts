  ...
  for (const summary of summaries) {
    ...
    if (rewardTokens.length > 0) {
      ...
      for (const symbol of rewardTokens) {
        ...
        console.log(
          `  ${formatTokenAmount(amount, symbol)} ${symbol} (${formatCurrency(usdValue)})${tokenPriceInfo}`,
        );
...
const displayBalanceHistoryTable = (summaries: DailySummary[]): void => {
  ...
  console.log(
    `Found ${summaries.length} historical daily records (table view):`,
  );
  console.log();
  ...
  // Build the header
  let header =
    "| Date       | Total USD    | autoUSD     | autoETH       | dineroETH     | FLP          |";
  let separator =
    "|------------|--------------|-------------|---------------|---------------|--------------|";
...
export const history = async (
  ...
  try {
    ...
    const walletAddress =
      options.address || address || process.env.WALLET_ADDRESS;
    ...
    if (options.range) {
      ...
      const dates = await balanceRecordService.getBalanceRecordsByType(
        walletAddress,
        BalanceType.TOTAL,
      );
