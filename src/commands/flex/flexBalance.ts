  ...
  try {
    ...
    for (const subAccountId of subAccountIds) {
      try {
        ...
        console.log(`  Unrealized PnL:      ${formatUsd(equity.unrealizedPnl)}`);

        // Leverage and margin
        console.log("\n⚖️  Leverage:");
        console.log(`  Current Leverage:    ${leverage.leverage.toFixed(2)}x`);
        console.log(`  Total Position Size: ${formatUsd(leverage.totalPositionSize)}`);

        // Available margin calculation
        const maxLeverage = 20; // Conservative default
        const maxPositionSize = equity.equity * maxLeverage;
        const availableMargin = Math.max(0, maxPositionSize - leverage.totalPositionSize);
        
        console.log(`  Available Margin:    ${formatUsd(availableMargin)} (at ${maxLeverage}x)`);
