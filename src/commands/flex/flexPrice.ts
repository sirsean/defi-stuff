...
export async function flexPrice(options: FlexPriceOptions): Promise<void> {
  try {
    ...
    if (options.all) {
      ...
      console.log(
        "💡 Tip: Use --market <SYMBOL> or --all to see other markets\n",
      );
    }
      ...
      try {
        ...
        if (options.all) {
          ...
          if (Math.abs(skewPercent) > 20) {
            console.log(
              `  ⚠️  Market is heavily skewed ${skewPercent > 0 ? "long" : "short"}`,
            );
          }
          ...
          console.log(
            `  • Longs pay shorts when market is net long, and vice versa`,
          );
