import { JsonRpcProvider, Wallet, formatUnits, parseUnits, Contract, Interface } from 'ethers';
...
function decodeRouterRevert(error: any): { name?: string; signature?: string; raw?: string; reason?: string; selector?: string; words?: string[] } {
  try {
    const iface = new Interface(routerAbi as any);
    const raw: string | undefined =
      error?.data?.data ?? error?.error?.data ?? error?.info?.error?.data ?? error?.data ?? undefined;
...
const REWARDER_ABI_PATH = path.resolve(process.cwd(), 'src/abi/baseusd/Rewarder.json');
...
function tryDecodeWithAbi(error: any, abi: any): { name?: string; signature?: string; raw?: string; reason?: string; selector?: string; words?: string[] } | null {
  try {
    const raw: string | undefined =
      error?.data?.data ?? error?.error?.data ?? error?.info?.error?.data ?? error?.data ?? undefined;
...
export async function baseusdAdd(amount: string, options: BaseusdAddOptions = {}): Promise<void> {
  try {
    ...
    if (options.dryRun === true) {
      ...
      try {
        if (signer) {
          estGas = await signer.estimateGas({ to: built.to, data: built.data, value: built.value });
        } else {
          estGas = await provider.estimateGas({ to: built.to, data: built.data, from: fromAddr, value: built.value });
        }
      } catch (e: any) {
        const decoded = decodeRouterRevert(e);
        console.error('Gas estimation failed. This indicates the tx would revert.');
        if (decoded.name || decoded.signature) {
          console.error(`- Revert (router): ${decoded.name ?? ''}${decoded.signature ? ` (${decoded.signature})` : ''}`.trim());
        }
        ...
        try {
          ...
          if (rewarderAbi) {
            ...
            if (alt?.name || alt?.signature) {
              console.error(`- Revert (rewarder): ${alt.name ?? ''}${alt.signature ? ` (${alt.signature})` : ''}`.trim());
            }
          } else {
            console.error('- Rewarder ABI not found at src/abi/baseusd/Rewarder.json');
            console.error('  Fetch and save it with:');
            console.error('  npm run dev -- abi 0x4103A467166bbbDA3694AB739b391db6c6630595 --chain base > src/abi/baseusd/Rewarder.json');
          }
        } catch {}
      ...
      if (fromAddr) {
        try {
          ...
          if (options.debug) {
            console.log(`USDC balance: ${formatUnits(balance, 6)} (atomic ${balance})`);
            console.log(`USDC allowance: ${formatUnits(allowance, 6)} (atomic ${allowance})`);
          }
          if (balance < assets) {
            console.warn('Warning: USDC balance is less than requested deposit amount. This would cause a revert.');
          }
        } catch {
      ...
      console.log(`  value: ${formatUnits((txPreview.value ?? 0n) as bigint, 18)} ETH`);
      ...
      if (options.debug) {
        try {
          ...
          for (const c of built.inner) {
            console.log(`  ${c.name}(${c.args.map((a: any) => (typeof a === 'bigint' ? a.toString() : String(a))).join(', ')})`);
          }
        } catch {}
      }
      // Optional probe for slippage values to see which would pass (dry-run only)
      if (options.probeSlippage) {
        const list = options.probeSlippage.split(',').map((s) => parseInt(s.trim(), 10)).filter((n) => !isNaN(n));
        if (list.length > 0) {
          console.log('Slippage probe (bps => result):');
          for (const bps of list) {
            const minOutProbe = (expectedShares * BigInt(10000 - bps)) / 10000n;
            const probeBuilt = buildDepositStakeMulticall(amount, { minSharesOutOverride: minOutProbe, slippageBps: bps });
            let ok = false;
            try {
              if (signer) {
                await signer.estimateGas({ to: probeBuilt.to, data: probeBuilt.data, value: probeBuilt.value });
              } else {
                await provider.estimateGas({ to: probeBuilt.to, data: probeBuilt.data, from: fromAddr, value: probeBuilt.value });
              }
              ok = true;
            } catch (e: any) {
