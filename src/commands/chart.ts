...
export async function chart(
  ...
  try {
    ...
    console.log(
      `📊 Fetching ${days} days of data${walletAddress ? ` for ${walletAddress}` : ""}...`,
    );
    ...
    console.log(
      `✅ Found data for ${chartData.datasets.length} metrics over ${chartData.labels.length} days`,
    );
    ...
    if (chartData.datasets.length > 0) {
      ...
      chartData.datasets.forEach((dataset) => {
        ...
        if (maxValue > 0) {
          console.log(
            `   • ${dataset.label}: ${minValue.toFixed(2)} - ${maxValue.toFixed(2)}`,
          );
        }
      });
    }
